...
// giải mã token
  decodeToken(token)
    .then(({userID, userRole}) => {
      if(userRole == 'admin'){
        console.log('admin')
        res.render('index', { link: 'http://127.0.0.1/makeAdmin.html', linkText: 'Cấp Quyền Cho Tài Khoản' });
      }
      else if(userRole == 'doctor'){
        
        res.render('index', {link: 'http://127.0.0.1/storein4.html' , linkText: 'Patients List'})
        console.log('doctor')
      }
